...
def metis_partition_groups_seeds_stellargraph(G: StellarGraph, maximum_seed_size, dump_graph=False):
    import subprocess
    import metis
    ...
    if dump_graph:
        ...
        for i, subV in enumerate(CC):
            print(f'Serializing subgraph {i}...')
            cur_subgraph_path = os.path.join(sub_graph_dir, f'cc_{i}xxx.graph')
            ...
            if n_edges == 0:
                ...
                # Replace `xxx` text with 2* number of edges 
                # sys_call = f'sed -e \'1s/xxx/{2*n_edges}/g\' {cur_subgraph_path} > {cur_subgraph_path}'
                out_file = open(cur_subgraph_path.replace('xxx', ''), "w")
                _ = subprocess.call(['sed', '-e', f'1s/xxx/{2*n_edges}/g', cur_subgraph_path], stdout=out_file)
                out_file.close()
                Path(cur_subgraph_path).unlink()
                # os.system(sys_call)
                print(f'Finish serializing subgraph {i}.')
                nparts = int( len(subV)/maximum_seed_size + 1 )
